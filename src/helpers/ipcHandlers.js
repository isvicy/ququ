...
class IPCHandlers {
  ...
  // 根据文本长度选择 prompt 策略
  _selectPromptStrategy(textLength) {
    if (textLength < 50) {
      return 'minimal';
  ...
  _buildOptimizePrompt(text) {
    const strategy = this._selectPromptStrategy(text.length);

    const prompts = {
      // 短句（<50字）：最小改动
      minimal: `修正明显错字和语音识别错误，其他保持原样。直接输出结果。
...
原文：${text}`,
      ...
      // 标准优化（50-300字）
      standard: `清理语音转录文本，保持原有风格。

规则：
1. 移除填充词（呃、嗯、那个、就是说、然后那个）
2. 处理重复（我我我→我）
3. 整合自我修正（周三不对周四→周四）
4. 修正明显错字
5. 保留语气词（啊、呀、呢、吧）
